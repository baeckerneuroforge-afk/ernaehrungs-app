...
export default function ScannerPage() {
  ...
  // Stable refs — survive re-renders, no DOM conflicts
  ...
  // Pre-check: can we even load html5-qrcode? (it needs browser APIs)
  useEffect(() => {
  ...
  const stopScan = useCallback(async () => {
    ...
    if (!scanner) return;
    try {
      // html5-qrcode getState: NOT_STARTED=0, PAUSED=1, SCANNING=2
  ...
  const startScan = useCallback(async () => {
    ...
    // Assign a stable ID (html5-qrcode requires an element ID string)
    const containerId = "barcode-scanner-region";
    container.id = containerId;

    // Ensure container is visible before starting (html5-qrcode measures the element)
    container.style.display = "";

    try {
      // Dynamic import — html5-qrcode uses browser APIs, must not run on server
      let Html5Qrcode;
      try {
        ...
        if (mountedRef.current) {
          setError("Scanner-Bibliothek konnte nicht geladen werden. Bitte Seite neu laden.");
        }
        return;
      }
      ...
      await scanner.start(
        ...
        async (decodedText: string) => {
          // Stop camera first, then update state
          try {
            await scanner.stop();
          } catch (e) {
      ...
      if (mountedRef.current) {
        ...
        if (msg.includes("NotAllowed") || msg.includes("Permission")) {
          ...
          setError("Die Kamera wird von einer anderen App verwendet. Bitte schließe andere Apps und versuche es erneut.");
        } else {
  ...
  const handleScanAgain = useCallback(async () => {
    ...
    setPortion(100);
    await stopScan();
    // Wait for html5-qrcode to fully tear down DOM nodes
      ...
      <main className="flex-1 max-w-lg mx-auto px-4 sm:px-6 py-10 w-full">
        ...
        {!scannerReady ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-ink-muted">Scanner wird vorbereitet...</p>
          </div>
        ) : (
          <>
            {/*
              Scanner container — ALWAYS in the DOM. html5-qrcode inserts its
              own <video>/<canvas> children here. We use `invisible h-0` instead
              of `hidden` so the element stays in the layout flow and can be
              measured by html5-qrcode when it starts.
            */}
            <div
              ref={scannerContainerRef}
              className={`w-full rounded-2xl overflow-hidden border border-border mb-4 ${
                scanning
