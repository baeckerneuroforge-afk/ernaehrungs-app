...
const PREMIUM_TEASER_MARKER = "[Premium entdecken →]";

export function ChatMessage({ content, isStreaming }: ChatMessageProps) {
  if (!content && isStreaming) return null;

  // Check if content contains the premium upgrade teaser
  const hasPremiumTeaser = content.includes(PREMIUM_TEASER_MARKER);

  if (hasPremiumTeaser) {
    const [mainContent, teaserRest] = content.split(PREMIUM_TEASER_MARKER);
        ...
        <div className="mt-4 rounded-xl border-2 border-primary/30 bg-primary-bg/20 p-4">
          ...
          >
            <span className="text-sm font-semibold text-primary">
              Premium entdecken &rarr;
            </span>
          </Link>
        </div>
        {teaserRest && <MarkdownContent content={teaserRest} />}
      </div>
    );
  }
